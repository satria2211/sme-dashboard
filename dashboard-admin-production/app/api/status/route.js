...
export async function POST(request) {
  try {
    ...
    // Check token early with better logging
    ...
    if (!token) {
      ...
      return NextResponse.json(
        { 
          ...
          details: 'BLOB_READ_WRITE_TOKEN environment variable is missing. Please add it in your Vercel project settings.'
        },
        { status: 500 }
      )
    }
    ...
    if (!token.startsWith('vercel_blob_rw_')) {
      console.error('❌ Token format invalid - should start with vercel_blob_rw_')
      return NextResponse.json(
        { 
          error: 'Invalid token format',
          details: 'BLOB_READ_WRITE_TOKEN should start with "vercel_blob_rw_". Make sure you copied the full token from Vercel Blob settings.'
        },
        { status: 500 }
      )
    }
    
    console.log('✅ Token found and format looks valid')
    ...
    try {
      ...
      console.log('Testing blob connection with metadata upload...')
      ...
      if (nplFile && nplData) {
        ...
        console.log('Uploading NPL metadata...')
        await put('npl_metadata.json', metadataContent, { 
          access: 'public',
          addRandomSuffix: false
        })
        console.log('✅ NPL metadata uploaded')
        
        console.log('Uploading NPL data...')
      ...
      const errMsg = blobError.message || ''
      ...
      if (errMsg.includes('403') || errMsg.includes('Forbidden')) {
        ...
        troubleshooting = [
          ...
          '3. Copy the full token (starts with "vercel_blob_rw_")',
          '4. Add to Environment Variables: BLOB_READ_WRITE_TOKEN',
          '5. Redeploy your application',
          '6. Make sure the token is enabled for Production/Preview/Development'
        ]
      } else if (errMsg.includes('401') || errMsg.includes('Unauthorized')) {
        errorMessage = 'Authentication failed (401 Unauthorized)'
        troubleshooting = [
          '1. Verify your BLOB_READ_WRITE_TOKEN is correct',
          '2. Token should start with "vercel_blob_rw_"',
          '3. Check if token has expired',
          '4. Regenerate token in Vercel Dashboard if needed'
        ]
      } else if (errMsg.includes('404')) {
        errorMessage = 'Blob store not found (404)'
        troubleshooting = [
...
export async function GET() {
  ...
  const response = NextResponse.json({
    ...
    hasToken: !!token,
    tokenFormat: token ? (token.startsWith('vercel_blob_rw_') ? 'valid' : 'INVALID - should start with vercel_blob_rw_') : 'missing',
